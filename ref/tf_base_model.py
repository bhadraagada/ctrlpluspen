...
class TFBaseModel(object):
    ...
    def __init__(
        ...
        # Configure GPU memory growth to fix cuBLAS issues on newer GPUs
    ...
    def fit(self):
        with self.session.as_default():
            ...
            if self.warm_start_init_step:
                logging.info('restoring from checkpoint step {}'.format(self.warm_start_init_step))
                self.restore(self.warm_start_init_step)
                step = self.warm_start_init_step
            else:
                logging.info('initializing all variables...')
                self.session.run(self.init)
                logging.info('variables initialized - starting training loop')
                step = 0
            ...
            while step < self.num_training_steps:
                ...
                val_metrics = dict(zip(list(self.metrics.keys()), val_metrics))
