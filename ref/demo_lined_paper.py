...
class Hand(object):
    ...
    def _draw(self, strokes, lines, filename, stroke_colors=None, stroke_widths=None):
        ...
        # Paper background - light cream/beige color
        dwg.add(dwg.rect(insert=(0, 0), size=(
            view_width, view_height), fill='#fefcf3'))

        # Add red vertical margin line on the left
        margin_x = 80
        dwg.add(dwg.line(
            start=(margin_x, 0),
            end=(margin_x, view_height),
            stroke='#ff6b6b',
            stroke_width=2
        ))

        # Add horizontal lines (ruled paper)
        for i in range(len(strokes) + 2):
            y_pos = i * line_height + line_height/2
            # Main line (darker blue-gray)
            dwg.add(dwg.line(
                start=(0, y_pos),
                end=(view_width, y_pos),
                stroke='#b8d4e8',
                stroke_width=1.5,
                opacity=0.6
            ))
