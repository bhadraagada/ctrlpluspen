...
### Windows (Conda) setup (recommended for this repo)

This repo includes legacy pinned dependencies (notably `numpy==1.17.4` and `matplotlib==2.2.2`). A Python 3.6 conda env is the most reliable way to install it.

```powershell
cd ref1
conda env create -f environment.win.yml
conda activate copycat-win
python main.py
```

Note: `gunicorn` is for Linux deployment and doesn't work on Windows; use `python main.py` for local runs.
